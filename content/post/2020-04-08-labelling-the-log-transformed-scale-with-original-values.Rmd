...
```
local y = ln(100)
local opt = "mlabel(country_lab) msize(tiny)  lcolor(%40) mcolor(%40) mlabsize(tiny)"

twoway ///
	(function y = .34657359*x + ln(100), range(0 20) lcolor(gs13))  ///
	(function y = .13862944*x + ln(100), range(0 50)  lcolor(gs13)) ///
	(function y = .06931472*x + ln(100), range(0 70)  lcolor(gs13))  ///
	(function y = .034657365*x + ln(100), range(0 70)  lcolor(gs13)) ///
	...
	(connected ln_confirmed date_100 if countryregion == "UK", sort `opt') ///
	...
	legend(off) yline(`y', lpattern(dash) ) ylab(`y_log'   `y' "100th case" ,  labsize(small) ang(h) grid) ///
	xlab(0(5)80, grid) ///
	xtitle("days since 100th confirmed case") ytitle("no. of confirmed (log-scale)") ///
	note("Source: JHU Github") ///
	scheme(s1color) 	///
	text(12. 15 "double every 2 days", size(tiny) color(%30)) ///
	text(12. 50 "double every 5 days", size(tiny) color(%30)) ///
	text(9 70 "double every 10 days", size(tiny) color(%30)) ///
	text(7 75 "double every 20 days", size(tiny) color(%30)) 
```


위 코드를 실행하면 아래와 같은 그림을 얻을 수 있다.

![](/post/2020-04-08-labelling-the-log-transformed-scale-with-original-values_files/country_log.png){ width=105% }
